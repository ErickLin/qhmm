...
if size(size(col_stoch), 2) ~= 2 || ~isequal(sum(col_stoch, 1), ones(1, size(col_stoch, 2)))
    ...
    m = size(col_stoch, 1);
    % take all nonzero elements and convert to fractional representation
    supp = sym(nonzeros(col_stoch));
    % size of middle/second subsystem which is the LCM of the denominators
    q = 1;
    for i = 1 : size(supp)
        [num, denom] = numden(supp(i));
        denom = double(denom);
        q = lcm(q, denom);
    end
%    quant_rep = zeros(n * q * m, n * q * m);
    ...
    for col = 1 : n
        ...
        for row = 1 : m
            ...
            for ntimes = 1 : q * col_stoch(row, col)
